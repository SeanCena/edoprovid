...
timeout = 10 * 60       # maximum recording time, in seconds
...
def main():
    ...
    loop_count = 0
    while loop_count < 3:  # Quits main loop after 3 * 20 seconds of idling
        try:
            # Poll queue
            ...
            if response is not None and response.get('Messages') is not None:
                ...
                s3.download_file(BUCKET_NAME, f'{file_id}.yrpX', os.path.join(edopro_path, f'replay/{file_id}.yrpX'))

                # Convert file
                convert_status = convert(file_id)

                # Upload mp4 if successful
                if convert_status:
                    s3.upload_file(f'/tmp/{file_id}.mp4', BUCKET_NAME, f'{file_id}.mp4')

                # Update outbound queue
                sqs.send_message(
                    QueueUrl=OUT_QUEUE_URL,
                    MessageBody=json.dumps({
                        'file_id': file_id if convert_status else '',
                        'file_name': file_name,
                        'owner': owner,
                        'channel': channel_id
                    }),
...
def convert(file_id):
    """Converts the replay file to mp4; returns true if successful and false if not"""
    return_status = True
    try:
        ...
        rec = subprocess.Popen(f'ffmpeg -y -video_size {w}x{h} -framerate 24 -f x11grab -i :0.0+{x},{y} -pix_fmt yuv420p -c:v libx264 -crf 35 -preset ultrafast -an /tmp/{file_id}.mp4', stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
        
        # Detect when replay is finished (the "replay ended" box appears) or replay times out
        ...
        while count < div:
            ...
            count = (count + 1) * (section == white)  # chat, is this marginally faster than an if-statement?
            cur_time = time.time()
